<template>
  <div>
    <Menu/>
    <el-button id="create" type="primary" size="small" @click="editCreateFormVisible = true">新增学生</el-button>
    ...
    />
    <el-dialog v-model="editCreateFormVisible" title="新增" align-center>
      ...
      <template #footer>
                <span class="dialog-footer">
                  <el-button @click="editCreateFormVisible = false">取消</el-button>
                  <el-button type="primary" @click="handleSave()">新增</el-button>
                </span>
      </template>
    </el-dialog>
    <el-dialog v-model="editUpdateFormVisible" title="编辑" align-center>
      ...
      <template #footer>
                <span class="dialog-footer">
                  <el-button @click="editUpdateFormVisible = false">取消</el-button>
...
//编辑框的可见性
const editUpdateFormVisible = ref(false)
const editCreateFormVisible = ref(false)
...
const handleUpdateForm = () => {
  update(updateForm).then(response => {
    if (response.code == 200) {
      success("修改成功")
      editUpdateFormVisible.value = false
      loadTableData()
    }
  })
}
...
const handleEdit = (row) => {
  editUpdateFormVisible.value = true
...
const handleSave = () => {
  save(createForm).then(res => {
    console.log(res)
    if (res.code == 200) {
      editCreateFormVisible.value = false
      loadTableData()
      success("新增成功")
    }
  })
